...
class ImageOnScrollListener private constructor(
    ...
    companion object {
        @JvmStatic
        fun create(tag: Any = ImageLoader.TAG_SCROLL_IMAGE, listener: AbsListView.OnScrollListener? = null): ImageOnScrollListener {
            return ImageOnScrollListener(tag, listener)
        }
    }
    ...
    private fun updateVanGoghStatus(idle: Boolean) {
        if (idle) {
            ImageLoader.resume(tag)
        } else {
            ImageLoader.pause(tag)
        }
    }
}
